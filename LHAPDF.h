...
namespace LHAPDF {
  ...
  inline std::ostream& operator<<(std::ostream& os, const LHAPDF::PDFSetInfo& info) {
    os << info.toString();
    return os;
  }

  /// Get a PDF set info object by filename and member number.
  LHAPDF::PDFSetInfo getPDFSetInfo(const std::string& filename, int memid);

  /// Get a PDF set info object by the LHAPDF ID number.
  LHAPDF::PDFSetInfo getPDFSetInfo(int id);

  /// Get a vector of PDF set info objects for all known sets.
  std::vector<LHAPDF::PDFSetInfo> getAllPDFSetInfo();
  //@}
