...
class grid
{
  ...
  public:
    ...
    bool setNormalised(bool t = true) {
      return m_normalised = t;
    }
    ...
    bool symmetrise(bool t = true) {
      return m_symmetrise = t;
    }
    ...
    const igrid* weightgrid(int iorder, int iobs) const {
      return m_grids[iorder][iobs];
    }
    ...
    int    Nobs() const {
      return m_obs_bins->GetNbinsX();
    }
    double obs(int obsbin) const {
      return m_obs_bins->GetBinCenter(obsbin + 1);
    }
    int    obsbin(double obs) const {
      return m_obs_bins->FindBin(obs) - 1;
    }
    double obslow(int iobs = 0) const {
      return m_obs_bins->GetBinLowEdge(iobs + 1);
    }
    double obsmin() const {
      return obslow(0);
    }
    ...
    double deltaobs(int iobs = 0) const {
      return m_obs_bins->GetBinWidth(iobs + 1);
    }

    TH1D*       getReference() {
      return m_obs_bins;
    }
    const TH1D* getReference() const {
      return m_obs_bins;
    }
    ...
    int  leadingOrder() const {
      return m_leading_order;
    }
    ...
    double getCMSScale() const {
      return m_cmsScale;
    }
    void   setCMSScale(double cmsScale) {
      m_cmsScale = cmsScale;
    }
    ...
    bool getApplyCorrections() const {
      return m_applyCorrections;
    }
    ...
    void sherpa() {
      m_type = SHERPA;
      std::cout << "appl::grid::sherpa()   using SHERPA convolution" << std::endl;
    }
    void amcatnlo() {
      m_type = AMCATNLO;
      std::cout << "appl::grid::amcatnlo() using aMC@NLO convolution" << std::endl;
    }
    void standard() {
      m_type = STANDARD;
      std::cout << "appl::grid::standard() using standard convolution" << std::endl;
    }

    CALCULATION  calculation() const {
      return m_type;
    }
  ...
  protected:
    ...
    static std::vector<std::string> parse(std::string s, const std::string& key) {
      ...
      while (s.size())
        clauses.push_back(chomptoken(s, key));
